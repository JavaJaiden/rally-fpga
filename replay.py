...
def export(trace: Path, output: Path) -> None:
    ...
    label = labels.pop()
    is_remote = label.startswith("REMOTE / ")
    intro = ("The player runs the game locally. A relay returns movement from its selected controller. "
             "The backend label identifies the declared mode."
             if is_remote else
             "The game checks each controller reply before applying a bounded paddle movement.")
    route = ("Player: game + remote client\n"
             "          | game state / checked movement\n"
             "          | SSH tunnel between machines\n"
             "Friend: relay -> UART -> FPGA controller\n\n"
             "The replay does not verify the network route or physical board."
             if is_remote else
             "Game -> state + sequence + CRC -> controller\n"
             "Game <- bounded move + status <- controller\n"
             "Rejected or missing reply -> zero movement")
    rows = []
    for r in records:
...
<h1>Rally: a remote controller for your game</h1>
...
<pre>__ROUTE__</pre>
<p class="foot">Rally uses the included game's state and control interface. Another game needs its own supported adapter. Playback speed is not a latency measurement.</p>
...
draw();requestAnimationFrame(tick);
    ...
    for key, value in {'ROUTE_TITLE': 'Deployment layout' if is_remote else 'Controller interface',
                       'ROUTE': html.escape(route), 'INTRO': html.escape(intro), 'LABEL': html.escape(label), 'MAX': str(len(rows)-1),
